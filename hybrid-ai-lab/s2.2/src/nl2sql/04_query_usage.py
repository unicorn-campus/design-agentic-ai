...
FROM card_txn AS t
JOIN card AS c ON ___1___
WHERE c.member_id = %(member_id)s
  AND t.txn_date >= date_trunc('month', %(base_date)s::date) ___2___
  AND t.txn_date < ___3___
  AND t.approval_code = '___4___'
GROUP BY ___5___
ORDER BY month ASC
LIMIT 12;
'''
...
def query(repository, member_id, base_date):
    require_complete(SQL, '04_query_usage.py')
