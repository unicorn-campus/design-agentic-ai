...
| 7 | [07-api-ui.md](07-api-ui.md) | API · 화면 — **조립 루트** · 진입점 · 스트리밍 · 프론트 · **포트**(설계 범위 밖) | ③ ② ⑦ | 클로니 |
...
| 6 | **Context7 MCP를 코드 작성 직전에 씀** | `mcp__context7__resolve_library_id` → `mcp__context7__query_docs` 순서로 최신 API 확인 |
| 7 | **부품을 만드는 프롬프트와 부품을 잇는 프롬프트를 나눔** — 1~6은 부품만 만들고 **잇는 자리는 7 한 곳**임 | 모두가 「누가 넣어 주겠지」로 끝내면 시험은 다 통과하는데 요청은 1건도 안 도는 앱이 나옴 |
...
| 값 | 정하는 프롬프트 | 어디에 남기나 |
|----|---------------|-------------|
| **조립 루트**(설정 · 커넥터 · 검사기 · 담당자 · 흐름을 실제로 만들어 진입 함수에 넣는 코드) | `07-api-ui.md` 5-1단계 | 서비스별 모듈 + README 조립 표 |
...
| 증상 | 원인 | 할 일 |
|------|------|------|
| 시험은 전부 통과하는데 **요청이 전부 「준비 미완료」로 응답함** | `07`이 조립 루트를 안 만들고 의존성을 인자로 열어 두기만 했음 | `07-api-ui.md` 5-1단계를 수행함. `06` README의 **진입 함수 의존성 목록**이 채울 항목의 근거임 |
| `06`이 끝났는데 `07`이 무엇을 채워야 할지 모름 | `06`이 진입 함수 의존성 목록을 README에 안 남겼음 | `06-workflow.md` 11단계의 의존성 목록 표를 먼저 채움 |
