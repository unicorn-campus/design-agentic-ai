...
def timeline():
    ...
    bottom = [('E-6', '위험도 산출'), ('E-7', '근거 묶음 선정'), ('E-8', '초안 작성'),
...
def storyboard():
    ...
    b.text(1102, 515, '상담 우선순위', 20, C['muted'])
    b.text(1102, 552, '우선 검토 · 점수 0.82', 24, bold=True)
    ...
    b.note('3단계는 화면 구상', '5단계에서 같은 U-3에 표시할 값과 출처를 읽기 모델로 연결함. 점수는 설계용 가상값.')
...
def contracts():
    ...
    columns = [
        ...
        ('예측 특징 준비', '문의·감정·반복·영향', 'C-6 · 예측형 AI', '상담 위험도를\n산출하라',
         'PR-DEMO/v1 추론', 'E-6', '상담 위험도 산출', '0.82 / 우선 검토'),
    ]
...
def merge():
    ...
    data = [('E-3 · 고객 사실', '전월 사용액 420,000원', '기준월 · 집계 정의'),
            ...
            ('E-6 · 검토 우선순위', '상담 위험 점수 0.82', 'PR-DEMO/v1 · 가상값')]
    for i, (label, title, detail) in enumerate(data):
        x = 50 + i * 380
    ...
    box(b, 50, 420, 1500, 150, 'C-7 · 규칙 기반 코드 → E-7', '동일 고객·상품·시점·권한 확인 → R-01 근거 묶음 선정',
        '원천 충돌·누락을 보존 / 예측은 별도 우선순위 / 추가 소비 거절을 제약으로 전달', 'orange', 25)
...
def specs():
    ...
    data = [
        ...
        ('C-6 · 예측', '모델 연결 실패', 'Given  모델 미연결 또는 시간 초과\nWhen   위험도 산출 요청\nThen   점수 없음 · 확인 필요 · 보류 사유'),
