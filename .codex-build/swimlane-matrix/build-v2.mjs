...
const FINAL_PPTX = path.join(workspaceDir, "docs/plan/think/em/Event-Modeling-작성예시-스윔레인매트릭스-v2.pptx");
...
function addSlice(row, col, offset, sliceId, title, commandId, eventId, compact = false) {
  ...
  const cardW = colW - 10;
  const titleH = compact ? 23 : 31;
  const metaH = compact ? 16 : 20;
  const badgeW = compact ? 39 : 43;
  addText({ name: `${sliceId}-badge`, text: sliceId, left: x, top: baseY, width: badgeW, height: titleH, fontSize: compact ? 12 : 14, bold: true, color: C.white, align: "center", fill: C.blue, radius: 5, insets: { left: 1, right: 1, top: 0, bottom: 0 } });
  addText({ name: `${sliceId}-title`, text: title, left: x + badgeW + 3, top: baseY, width: cardW - badgeW - 3, height: titleH, fontSize: compact ? 12 : 14, bold: true, color: C.navy, align: "left", fill: C.white, lineFill: C.blue, lineWidth: 0.8, radius: 4, insets: { left: 4, right: 2, top: 0, bottom: 0 } });
  addText({ name: `${sliceId}-links`, text: `${commandId} · ${eventId}`, left: x, top: baseY + titleH + 2, width: cardW, height: metaH, fontSize: compact ? 12 : 13, bold: true, color: C.slate, align: "center", fill: C.tint, lineFill: C.border, lineWidth: 0.7, radius: 4, insets: { left: 2, right: 2, top: 0, bottom: 0 } });
}
...
addSlice(3, 2, 5, "S-04", "질문 관련 문단 검색", "C-4", "E-4", true);
addSlice(3, 2, 51, "S-05", "보유 카드 관련 근거 조회", "C-5", "E-5", true);
